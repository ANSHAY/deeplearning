...
def load_hyperparameters(filename):
    ...
    try:
        ...
        metadata = {'hidden_activation': 'binary', 'final_activation': 'binary',\
            'lamda': 0, 'iterations': 3000, 'init_type': 'xavier',\
            'learning_rate': 0.1, 'layer_sizes': [64, 32, 8, 4, 1]}
        f = open(filename, 'wb')
...
def train_model(train_X, train_Y, metadata, dataset):
    ...
    try:
        f = open('models/nn_model_'+ metadata['hidden_activation'] + '_' +\
                 str(metadata['iterations']) + '_' +\
                 str(len(metadata['layer_sizes'])) + '.pkl', 'rb')
        model = pickle.load(f)
        f.close()
        print ('\nModel found................')
    except:
        ...
        f = open('models/nn_model_'+ metadata['hidden_activation'] + '_' +\
                 str(metadata['iterations']) + '_' +\
                 str(len(metadata['layer_sizes'])) + '.pkl', 'wb')
        pickle.dump(model, f)
        f.close()
    return model
...
if __name__ == '__main__':
    ...
    f.close()
    for i in range(1, 12):
        image_path = 'images/cat ('+str(i)+').jpg'
